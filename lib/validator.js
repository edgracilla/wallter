  ...
  validate (loc, path, value, schema, oldPath) {
    ...
    return BPromise.each(Object.keys(schema), method => {
      ...
      if (_.isFunction(validator[method])) {
        if (method === 'isLatLong') {
          // mongodb geospatial asks for [lng, lat] format; isLatLong asks for lat, lng format
          if (Array.isArray(opt[0]) && opt[0].length > 1) opt[0] = `${opt[0][1]},${opt[0][0]}`
        }
        ...
        if (!_.isString(opt[0]) || !validator[method](...opt)) {
          errBag.push(this.formatErr(data))
        }
      } else {
        if (method !== 'required') {
          if (_.isFunction(this.customValidators[method])) {
            return this.customValidators[method](...opt).catch(() => {
              errBag.push(this.formatErr(data))
            })
          }
        }
